...
4. **Kiểm tra trước khi gửi**: `npm run lint`, `npx tsc --noEmit`, `npm run test:agent`. Thay đổi AI engine thì chạy thêm `ai-engine/eval_ppe_decision.py` và `ai-engine/sweep_threshold.py` (xem README mục "File model cần có").
