...
# Cần: GH_TOKEN (scope repo), git credential để push. Dùng: GH_TOKEN=... scripts/github-open-issues-prs.sh [slug ...]
...
for slug in "${slugs[@]}"; do
  ...
  if git show-ref --verify --quiet "refs/heads/$branch"; then
    git push -u origin "$branch" >/dev/null 2>&1 && echo "đã push $branch" || echo "push $branch thất bại (kiểm tra credential)"
