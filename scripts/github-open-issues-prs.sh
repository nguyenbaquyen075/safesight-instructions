...
# Mỗi slug: docs/github/issues/<slug>.md (dòng đầu "# <tiêu đề>", dòng 2 tuỳ chọn "branch: <nhánh>" — mặc định feat/<slug>; phần còn lại là nội dung) và
...
BASE="${BASE:-main}"
RELEASE_LABEL="${RELEASE_LABEL:-v0.11}"
...
for slug in "${slugs[@]}"; do
  issue_file="docs/github/issues/$slug.md"
  [ -f "$issue_file" ] || { echo "bỏ qua $slug: thiếu $issue_file"; continue; }
  title=$(head -1 "$issue_file" | sed 's/^# *//')
  branch=$(sed -n '2p' "$issue_file" | sed -n 's/^branch: *//p'); [ -n "$branch" ] || branch="feat/$slug"
  pr_file="docs/github/prs/${branch//\//-}.md"
  body=$(tail -n +2 "$issue_file" | sed '1{/^branch: /d}')
  ...
  if [ -n "$existing" ]; then num=$existing; echo "issue #$num đã có: $title"; else
    num=$(api POST "/issues" -d "$(python3 -c 'import json,sys; print(json.dumps({"title": sys.argv[1], "body": sys.argv[2], "labels": ["enhancement", sys.argv[3]]}))' "$title" "$body" "$RELEASE_LABEL")" | python3 -c "import json,sys; print(json.load(sys.stdin)['number'])")
