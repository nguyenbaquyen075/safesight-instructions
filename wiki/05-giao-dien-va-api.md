...
| `/api/actions` | GET | Cần session; `?siteId&status=open\|overdue&limit` (mặc định 200, trần 500). Luôn chỉ trả việc `OPEN`; `overdue` lọc thêm `dueAt < now`. Không có `siteId` thì lọc theo `allowedSiteIds()`; xin site ngoài phạm vi → 403. Nguồn của mục "Việc khắc phục" ở `/reports` |
