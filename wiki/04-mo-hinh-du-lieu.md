...
| `Violation` | Vi phạm AI đã chốt | `type`, `severity`, `confidence`, `bboxData` (JSON), `snapshotUrl`, `clipUrl` (clip bằng chứng ~8s do engine ghi, NULL khi không ghi được), `occurrenceCount` (lần thứ mấy của cùng một người, reset khi rời khung; GET `/api/violations` và `/api/violations/[id]` trả về trường này), `agentReview` (JSON `{ verdict, band, observations[], note, sessionId, reviewedAt }`, agent ghi sau khi review), `reviewFeedback` (JSON `{ correct, note?, userId, userName?, at }` — `userName` là tên (hoặc email) người chấm, dòng cũ trước v0.11 không có nên giao diện hiện "Đã đánh giá lúc …"; người chấm phán quyết của agent đúng hay sai qua `PATCH /api/violations/[id]/feedback`, chỉ giữ lần chấm mới nhất; nguồn của `GET /api/stats/agent-accuracy` và bộ dữ liệu retrain) |
