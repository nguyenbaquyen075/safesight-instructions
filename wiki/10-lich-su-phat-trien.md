...
| 2026-09-08 | v0.10.0 | Widget **Trợ lý SafeSight** nổi trên mọi trang (chat dạng bong bóng, dùng chung với `/agent` và tab Agent trong modal); **agent trưởng điều phối subagent camera** (`list_camera_agents`, `dispatch_to_camera`, kind `camera.instruction`); Code of Conduct; đợt review health + change trước release (`docs/review/2026-09-08-*`) và fix: lọc trạng thái vi phạm chữ HOA, tool điều phối chỉ cho phiên toàn hệ thống, widget theo quyền `/agent`, toast dời lên trên |
| 2026-09-08 | v0.11.0 | **Lộ trình v0.11** (issue #22–#32, PR #23–#34 qua `integration/v0.11`, subagent theo spec/plan + review từng task + review toàn nhánh + fix wave): cảnh báo **xâm nhập vùng cấm / dưới tải treo** (engine + trình vẽ vùng theo loại); **loa công trường tự động** (bridge `/announce`, tool `announce`, nút "Phát loa"); **CAPA** giao xử lý vi phạm với hạn và leo thang quá hạn; **phản hồi Đúng/Sai** cho phán quyết agent, thẻ độ chính xác, xuất CSV retrain; leo thang vận hành qua mọi kênh; `ai-engine/requirements.txt` |
